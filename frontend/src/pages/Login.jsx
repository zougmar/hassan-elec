...
const LOGIN_TYPES = {
  ADMIN: 'admin',   // Admin or Manager
  EMPLOYEE: 'employee'
};

const Login = () => {
  const { t } = useTranslation();
  const { login, employeeLogin, user, isAuthenticated, loading: authLoading } = useAuth();
  ...
  const handleSubmit = async (e) => {
    ...
    const doLogin = loginType === LOGIN_TYPES.EMPLOYEE ? employeeLogin : login;
    const result = await doLogin(formData.email, formData.password);

    if (result.success) {
      toast.success('Login successful');
      if (loginType === LOGIN_TYPES.EMPLOYEE) navigate('/employee', { replace: true });
      else navigate('/admin', { replace: true });
    } else {
  ...
  const title = loginType === LOGIN_TYPES.EMPLOYEE ? 'Employee' : 'Admin / Manager';
  const subtitle = loginType === LOGIN_TYPES.EMPLOYEE
    ? 'View your tasks and update status.'
    : 'Manage services, projects, and requests.';
      ...
      <div className="flex-1 flex items-center justify-center p-6 sm:p-12 bg-slate-50 dark:bg-slate-900">
        <div className="w-full max-w-md">
          <div className="lg:hidden flex items-center justify-center gap-2 mb-8">
            ...
            />
            <span className="text-lg font-bold text-slate-800 dark:text-white">Hassan Elec</span>
          </div>

          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl shadow-slate-200/50 dark:shadow-none border border-slate-200/60 dark:border-slate-700 p-8 sm:p-10">
            {/* Switch: Admin/Manager | Employee */}
            <div className="flex rounded-xl bg-slate-100 dark:bg-slate-700 p-1 mb-8">
              <button
                type="button"
                onClick={() => setLoginType(LOGIN_TYPES.ADMIN)}
              ...
              <button
                type="button"
                onClick={() => setLoginType(LOGIN_TYPES.EMPLOYEE)}
                className={`flex-1 flex items-center justify-center gap-2 py-2.5 px-4 rounded-lg text-sm font-medium transition-all ${
                  loginType === LOGIN_TYPES.EMPLOYEE
                    ? 'bg-white dark:bg-slate-600 text-slate-900 dark:text-white shadow'
                    : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'
                }`}
              >
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-12 pr-4 py-3 rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50/50 dark:bg-slate-700 text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all duration-200"
                    placeholder={loginType === LOGIN_TYPES.EMPLOYEE ? 'employee@example.com' : 'admin@example.com'}
                  />
                </div>
              </div>
